...
private:
  ...
  std::unordered_set<state<obj_t>> _states;
  state<obj_t> _current_state;
...
public:
  finite_state_machine<obj_t>(obj_t &&object,
      ...
      : _object(std::forward<obj_t>(object)), _states(states) {
  ...
  void process(std::any input) { _current_state.process(input); }
